...
def test_list_parses_sequence_rows() -> None:
    client = _client()
    fake = _FakeResponse(
        {"items": [{"id": "a", "sequence": 1}, {"id": "b", "sequence": 2}], "version": 3}
    )
    with patch("urllib.request.urlopen", return_value=fake) as urlopen:
        rows = client.list("bindery")

    assert rows == [SequenceRow(id="a", sequence=1), SequenceRow(id="b", sequence=2)]
...
def test_sync_natural_posts_items() -> None:
    client = _client()
    fake = _FakeResponse({"items": [{"id": "a", "sequence": 1}], "version": 1})
    with patch("urllib.request.urlopen", return_value=fake) as urlopen:
    ...
    assert rows == [SequenceRow(id="a", sequence=1)]
    req = urlopen.call_args.args[0]
